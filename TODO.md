...
- 状态：Desktop Notes MVP 的直接产品 hosted 门禁已闭环（PR run `31902303937` 的 41/41 jobs 覆盖 Windows UIA、双平台真实 picker/clean-runner、安全门禁、v1 active 性能预算与 G6-05 unsigned input；run `31958001217` 已完成 v2 baseline activation，等待 active 状态下的 required 复核；clean-tag schema-v5 晋级、registry clean-user、真实签名/公证和最终发布仍待外部条件）
- 规划输入：`mvp@f3afbeb`
...
- [x] G6-07B2：以 RED→GREEN 合同测试冻结真实 `Presented` raw event、`GITHUB_SHA`/platform/runner/artifact identity、drop/timeout/缺样本/本地证据 fail-closed，以及 artifact 前后不变和 frame identity 一致性；验证：性能 Node 合同 `28/28`、Rust probe `3/3`。
...
- [x] G6-07B4：实现跨平台 Node collector；固定 3 warmup + 10 measured、每 measured 进程 1 startup + 100 steady-state（共 1000 个 steady 样本）、monotonic first-present、5 秒后 RSS、完整 frame durations、artifact bytes/SHA-256、commit binding、identity consistency、硬终止和 fail-closed report；验证：`node --test tools/performance-collector.test.mjs` 与 budget 合同合计 `28/28`，并覆盖固定 startup 边界、后续慢帧保留、active baseline 绑定、严格 CLI 参数和 collection/termination 双错误传播。
...
- [ ] G6-07P：run `31952821895` 暴露 first-present 混入 p95 的结构性不稳定后，workload 已递增为 v2；run `31958001217` 的双平台 steady-state raw report 已完成 identity/sample/quality/digest 评审、activation 与严格 active-budget 复算，等待 baseline active 状态下的 `performance-required` hosted 复核。
...
G6 hosted 状态复核（2026-08-16）：PR run `31902303937` 的 41/41 jobs 已绑定 source head `e56bb9e2c531e9cd3d97837465eca92d5e2c31dd`；Actions 中的 package/performance/artifact 执行则绑定 merge revision `991b28142783833c14be659125c4564d14219cc5`。该 run 关闭 Windows UIA、双平台 clean artifact/picker、hosted supply-chain、v1 performance 与 unsigned G6-05 input 的直接门禁。run `31958001217` 又绑定 source head `3051815dd70118c292ad7f64d1c34b0f728182c7` / merge revision `184351135c649f83af07730bf337ff9b20f8b89f`，由 jobs `95191688326` / `95191688333` 完成 v2 activation，下一次 `performance-required` run 负责关闭 G6-07P；它不是 clean tag，且没有 credential activation、registry clean-user、signed staging 或最终 publication，因此这些外部门禁继续保持未完成。
