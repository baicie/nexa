...
- [ ] G6-07P：v1 已由 runs `31895582357` / `31902303937` 完成 activation 与 required 复核；run `31952821895` 暴露 first-present 混入 p95 的结构性不稳定后，workload 已递增为 v2，等待双平台 steady-state raw report 评审、activation 与 required 复核。
...
G6 hosted 状态复核（2026-08-16）：PR run `31902303937` 的 41/41 jobs 已绑定 source head `e56bb9e2c531e9cd3d97837465eca92d5e2c31dd`；Actions 中的 package/performance/artifact 执行则绑定 merge revision `991b28142783833c14be659125c4564d14219cc5`。该 run 关闭 Windows UIA、双平台 clean artifact/picker、hosted supply-chain、v1 performance 与 unsigned G6-05 input 的直接门禁；v2 performance 需要新证据。它不是 clean tag，且没有 credential activation、registry clean-user、signed staging 或最终 publication，因此这些外部门禁继续保持未完成。
