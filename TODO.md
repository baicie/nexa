...
- [ ] G6-04P：在受信 hosted run 实际完成 registry npm audit、四份 Cargo lock advisory、deny 与 gitleaks，并记录绑定 revision 的结果。
...
G6-04 当前状态（2026-08-11，2026-08-13 Windows 闭包补充）：`security.yml` 和本地 negative fixtures 已实现并由 Action SHA policy 锁定；安全合同会拒绝未固定 Action、非法/过期 license exception 和 synthetic secret。根 workspace 原有的 `winit` 默认 feature 经 `wayland-csd-adwaita -> sctk-adwaita -> ab_glyph -> owned_ttf_parser -> ttf-parser` 引入 `RUSTSEC-2026-0192`；现改为显式保留 `rwh_06`、Wayland、Wayland 动态加载、X11 与无标题文本的 Adwaita CSD，并从根/UI Host lockfile 移除废弃字体解析链。Windows unified static closure 只启用参考应用需要的 Perry `core` 与 Host Promise 所需 `async-runtime` feature，独立 lock 已纳入 audit、deny 与 Dependabot；四份 lockfile 的 `cargo audit 0.22.2 --deny warnings` 与四个 manifest 的 `cargo-deny 0.20.2` advisories/bans/licenses/sources 已在本地通过。由于当前仍没有绑定 revision 的 hosted npm registry audit、Cargo advisory/deny 与 gitleaks 成功记录，G6-04 主项保持未完成。
