...
- [ ] G6-07P：run `31991801398` attempt 1 已取得并逐字节复核双平台各 3 份独立 hosted raw report、完整 report set 和共享 candidate identity，12 项 `reference-notes-v3` baseline 已激活；等待 baseline active 状态下的新 `performance-required` hosted 复核关闭本项。
...
G6 hosted 状态复核（2026-08-17）：PR run `31902303937` 的 41/41 jobs 已绑定 source head `e56bb9e2c531e9cd3d97837465eca92d5e2c31dd`；Actions 中的 package/performance/artifact 执行则绑定 merge revision `991b28142783833c14be659125c4564d14219cc5`。该 run 关闭 Windows UIA、双平台 clean artifact/picker、hosted supply-chain、v1 performance 与 unsigned G6-05 input 的直接门禁。v2 run `31958001217` 作为历史 steady-state 证据保留；当前 v3 capture run `31991801398` attempt 1 绑定 source head `8302b89200698df6a6ead7772928107ba4d56eee` / merge revision `2c91ac02340245e5bad206bc1e9fe0d8b96b88fd`，11/11 performance jobs 生成并复核六份 raw 和两个 report set，完成 v3 activation。下一次 `performance-required` run 负责关闭 G6-07P；capture run 不是 clean tag，且没有 credential activation、registry clean-user、signed staging 或最终 publication，因此这些外部门禁继续保持未完成。
