...
- 状态：Desktop Notes MVP / Technical Preview 本地收口中（G5-11 的唯一 Tier-1 Solid 与 G6-06 reviewed signing executor 已有本地证据；Windows UIA、双平台真实 picker/clean-runner、registry clean-user evidence、签名凭据/hosted staging 和性能 active baseline 仍待完成）
- 规划输入：`mvp@f3afbeb`
...
| G3B-05 | [ ] 建立语义驱动 E2E                         | 真实 NSAccessibility/UI Automation client 用 role/name 定位和操作 Notes，经 Adapter/Dispatcher 落地 | macOS/Windows native client smoke | G3B-03、G3B-04 | E2E harness、scenario、platform client、reference fixture                       | M    |
...
G3B-05 native client smoke 进展（2026-08-06）：新增小型原生 fixture，通过 production `accesskit_winit::Adapter`、winit user event 与 `Dispatcher<AccessibilityActionRequest>` 落地 action；它和 Minimal TSX/Bridge harness 组成分层证据。macOS 本机的进程内 NSAccessibility 对象 client 已连续通过 role/name Focus、SetValue、Invoke、焦点/value/Status 回读；native crate `2/2`、macOS strict Clippy 通过。Windows UI Automation COM client 已实现相同 control-type/name 工作流、35 秒 watchdog 与焦点回读，`x86_64-pc-windows-msvc` strict Clippy 通过。macOS 当前层不覆盖跨进程 `AXUIElement`/TCC 兼容矩阵。

G3B-05 剩余门禁：必须由 GitHub Windows runner 实际执行真实 UI Automation client smoke 并保留成功记录；当前仅有客户端实现、交叉编译和 Clippy 证据。该证据出现前 G3B-05 与 G3B 保持未完成。
...
- [ ] Windows UI Automation client 在 GitHub Windows runner 通过同一路径
...
- [x] G4-07f：在 Notes trusted launcher 接入真实 Clipboard read/write/read Promise fixture、严格 proof runner 与双平台 package workflow；本地只验证 runner `7/7` 和 Perry AOT/link，平台执行仍等待 hosted run。

G4-07 evidence（2026-08-07，2026-08-10 trusted launcher 补充）：Core injected backend `2/2`、System Host Rust `22/22`（20 unit + 2 integration）、Clipboard TS contract `4/4` 通过；`@nexa/clipboard` 不再调用 legacy sentinel，所有失败均通过 `NexaSystemError` 结构化传递。`smoke:clipboard` 使用 Notes 内嵌 manifest，在 `Ready` 后经 native worker、`SystemCompletion` 与 Perry Promise 执行 read/write/read，并在原文本可读时恢复；runner 要求精确 round-trip/restore proof，失败、假 marker 与提前退出均 fail closed。runner 合同 `7/7`、本机 Perry AOT/link 通过，真实 macOS/Windows 执行已接入 package matrix 但尚无 hosted 成功记录。
...
G5-03/G5-04 progress（2026-08-08，2026-08-09 加固）：实际 Notes 组件复用可注入 controller；`tools/notes-app.test.mjs` 15/15 覆盖 dirty、save dialog cancel、open/save/error、foreground exclusion、revision snapshot、suspend/resume、pending read/write close 与 late completion。`tools/reference-notes-e2e.test.mjs` 4/4 在实际 Notes TSX 树中暴露 Open、Save、Title、Body、Status 五个稳定语义节点并执行 Focus/SetValue/Invoke，验证 busy 时 role/name 不漂移、幂等 `Ready/Suspended/Resumed/CloseRequested`、pending write close、dirty 状态下 typed `PERMISSION_DENIED` 和完整诊断对象。无 fixture real-picker probe 顺序要求 save/open/cancel、版本化 snapshot 与磁盘 bytes，runner/driver 合同 24/24；Windows probe 以 suspended 状态加入 kill-on-close Job Object 后才恢复并回传真实 PID，POSIX 在 child close 后再对 PGID 强杀确认；无法确认终止时有界失败并保留自建目录。G5-03 仍等待 Windows UI Automation runtime；G5-04 仍等待 macOS/Windows 真实 `rfd` picker 返回 selection/cancel 并完成 Promise 的成功记录。
...
- [ ] G5-03P：在绑定当前 commit 的 Windows hosted runner 记录 UI Automation runtime 成功证据。
- [ ] G5-04P：在绑定当前 commit 的 macOS 与 Windows 图形会话记录真实 picker save/open/cancel 成功证据；记录 run、job、OS/arch 与 probe binary digest。
...
四项本地合同均通过。它们关闭 G5-11，不关闭 G5-03/G5-04/G5-09/MVP-01，也不构成 registry publish、签名或 hosted 平台证据。
...
G5-09 本地实现证据（2026-08-08，2026-08-11 runtime proof 补充）：create-to-package smoke 支持 CI 专用 `--artifact-output <path>`，精确 destination 必须不存在，临时工程与输出位于同一 parent，全部 package 校验后才移动，并保持无参数默认清理。package 合同当前 23/23、CLI 聚合 57/57、根聚合 268/268；CI route/workflow fail-closed 合同 22/22。`reference-notes-package.yml` 已拆为双平台 build/archive/upload 与 fresh download/validate/launch 两个 job，build 还强制运行无 fixture real-picker probe并持久化 revision/ref/run/target-bound native runtime proof，绑定 FS/picker Perry binary 的 size/SHA-256 与 N-04/N-05/N-06/G5-04P 固定旅程；launch 不 checkout、setup、install 或 rebuild，下载后重新校验 manifest、metadata、target、version、asset bytes、runtime proof 与 macOS plist，并分别启动通用与 Notes executable。真实 macOS 通用 artifact 的导出、`tar.gz` 往返、executable mode 与 5 秒启动已在本地验证；这些本地证据仍不替代 macOS/Windows real-picker 或 artifact/launch hosted 成功记录。
...
真实 native FS、Clipboard launcher 与 deterministic Dialog 证据（2026-08-08，2026-08-10 Clipboard 补充）：`smoke:fs` 用受信内嵌 manifest 编译并启动 Perry 进程，经 Notes controller 保存后在临时目录写入并读回 `Nexa UI UTF-8 smoke: 你好, مرحبا, 😀`；native worker、SystemCompletion 和 Perry Promise continuation 完成后输出 `savedRevision == revision`、`dirty == false` 的状态证明，Node runner 再核对磁盘 bytes。`smoke:clipboard` 已接入同一 trusted manifest 和双平台 workflow，要求 native read/write/read 精确 proof，并尽可能恢复原文本；本地 runner 合同 `7/7` 与 Perry AOT/link 通过，尚未取得 hosted runtime 成功记录。`smoke:dialog` 在真实 Perry/native System Host 中使用构建期注入且有序的 Dialog backend，完成 save 选择与磁盘写入、open 选择与文件读入、第二次 open 的 `null` cancel，并核对三个精确 controller snapshot；runner 正反合同 7/7。Dialog 证据证明 Task/Promise/应用状态链路，不等同于真实 OS picker。

真实 picker probe 证据（2026-08-08，2026-08-09 加固）：`smoke:picker` 的编译和运行环境均大小写不敏感地删除 `NEXA_DIALOG_TEST_FIXTURE_PATH`，编译后二进制拒绝 deterministic fixture canary 并输出 SHA-256；runner 必须依序驱动 save/open/cancel，核对真实磁盘 bytes 和版本化 Notes controller snapshot，并让 driver、stage、proof、marker、timeout、关闭等待或 cleanup 任一失败阻断。runner/driver 合同 24/24，覆盖 PowerShell `Add-Type` 和所有跨进程消息在总预算内、Windows foreground/control/有界 `WM_GETTEXT` readback、suspended probe/Job Object/PID handoff、POSIX PGID 最终强杀、TERM-to-KILL 升级、强杀确认期限、close 后清理和 setup/cleanup 双错误；未确认终止会保留现场目录。本机 `System Events -> UI elements enabled = false`，实际运行精确失败于 Accessibility permission unavailable，且无残留进程/临时目录；这不是 selection/cancel 成功证据。

MVP-01 progress（2026-08-08，2026-08-11 runtime proof 补充）：Notes build wrapper 合同 5/5，真实主应用 AOT/startup smoke 通过；正式构建从所有 child process 清除 `NEXA_DIALOG_TEST_FIXTURE_PATH` 并拒绝包含 fixture marker 的二进制。内部 packager 合同 6/6，sidecar manifest 必须与二进制内嵌 manifest 完整 bytes 相同且拒绝 fixture 污染；本地 macOS `.app` 已生成、校验并启动。`reference-notes-package.yml` 已配置 `macos-15`/`windows-2022` build/upload 与独立 fresh download/launch matrix，执行 CLI create-to-package、真实 FS、真实 Clipboard Promise、构建期注入 Dialog Promise、无 fixture real picker、package、bundle/distribution 校验和两类 executable 的 5 秒启动存活检查；package/launch 之间还传递 native runtime proof，clean-package proof 内嵌 canonical payload 并再次绑定 size/SHA-256。CI route/fail-closed 合同保持通过。该 workflow 尚无 macOS/Windows Clipboard/picker 和双平台 artifact/launch hosted 成功记录，因此 MVP-01 和 G5-09 保持未完成；已配置 matrix 不等于已有 hosted-runner 成功记录。

## Desktop Notes MVP Gate

| ID     | Todo                               | Acceptance                                                                        | Verify                                        | Dependencies   | Files                                                          | Size |
| ------ | ---------------------------------- | --------------------------------------------------------------------------------- | --------------------------------------------- | -------------- | -------------------------------------------------------------- | ---- |
| MVP-01 | [ ] 生成 Notes unsigned 双平台产物 | macOS `.app` 与 Windows 分发目录不依赖 Node/Rust/仓库源码，可在 clean runner 启动 | package smoke + platform matrix               | G5-04、G5-09   | packager core、bundle layout、resource copier、workflow、tests | M    |
...
- [ ] 编辑、IME、A11y、open/save、cancel/close、suspend/resume 形成同一应用旅程
...
- [ ] `docs/MVP.md` Definition of Done 全部满足
...
- [ ] G6-04P：在受信 hosted run 实际完成 registry npm audit、三份 Cargo lock advisory、deny 与 gitleaks，并记录绑定 revision 的结果。
...
- [ ] G6-07P：采集真实 hosted cold start/RSS/tick/layout/paint 报告，评审并激活 macOS/Windows baseline。
...
G6-04 当前状态（2026-08-11）：`security.yml` 和本地 negative fixtures 已实现并由 Action SHA policy 锁定；安全合同会拒绝未固定 Action、非法/过期 license exception 和 synthetic secret。根 workspace 原有的 `winit` 默认 feature 经 `wayland-csd-adwaita -> sctk-adwaita -> ab_glyph -> owned_ttf_parser -> ttf-parser` 引入 `RUSTSEC-2026-0192`；现改为显式保留 `rwh_06`、Wayland、Wayland 动态加载、X11 与无标题文本的 Adwaita CSD，并从根/UI Host lockfile 移除废弃字体解析链。三份 lockfile 的 `cargo audit 0.22.2 --deny warnings` 与三个 manifest 的 `cargo-deny 0.20.2` advisories/bans/licenses/sources 已在本地通过；同时修复 `security.yml` 把全局 `--config` 错放在 `check` 后的问题，合同 RED 后 9/9 GREEN。由于当前仍没有绑定 revision 的 hosted npm registry audit、Cargo advisory/deny 与 gitleaks 成功记录，G6-04 主项保持未完成。

G6-05 evidence（2026-08-11）：`tools/release-dependency-graph.mjs` 从结构化 `pnpm list --prod --json --depth Infinity` 与两个 Host 的 `cargo metadata --locked` 收集生产闭包，并按 `release/packages.json` 的当前 9 个 npm 根与 2 个 Cargo Host 根校验。九包 consumer 实际得到 389 个 dependency-graph 组件和 389 个 dependency entries，CycloneDX 1.6 输出 398 个组件和 390 个 dependency entries。`SHA256SUMS`、SBOM 与 SLSA provenance 绑定同一 artifact、revision、builder 和 lockfile digest；fresh verifier 不访问 registry，并对额外/缺失/篡改 artifact、未知 edge、material digest drift 与非规范 sidecar fail closed。`reference-notes-package.yml` 现从两个平台 job 产出规范命名的 macOS/Windows unsigned archive，并由独立 `signing_input` job 组装、复核并上传 canonical `unsigned-signing-input`（revision-bound custody、G6-05 descriptor 和三份 sidecar）；producer 合同 `7/7`。这些本地生成合同不是 hosted provenance、registry、签名或公证证据。

G6-06/07 当前状态（2026-08-11）：`tools/signing-executor.mjs` v1.2.0 与 `tools/signing-credentials.mjs` 已由 `release/signing-policy.json` 绑定 entrypoint/closure SHA-256、runner 与平台命令路径；executor 在复制 unsigned bytes 或激活 credential 前强制验证规范 G6-05 descriptor、`SHA256SUMS`、CycloneDX 1.6 SBOM 与 provenance，custody schema v3 再绑定这些输入，publisher 也会独立复核双平台使用同一个完整输入 bundle。`signing.yml` 已按 `unsigned_run_id` 校验 `reference-notes-package.yml` producer run/revision、下载 canonical bundle，并编码 credential activation、签名、custody export/merge、signed manifest 与 publisher verification。policy/executor/credential/workflow contracts 本地通过；但两个 signing platform job 仍有 literal `if: ${{ false }}`，execution/credential activation 为 `disabled`，三个 owner 仍为 `UNASSIGNED_*`，protected Environment/真实凭据未配置，staging record 仍为 `pending`，因此 readiness 按设计失败，未发生真实 credential activation、签名/公证/时间戳或 hosted staging，G6-06 主项保持未完成。性能 native probe、collector 和 workflow 已完成本地实现与真实 macOS AOT smoke；两个平台 baseline 全部保持 `pending`，本地结果不进入 baseline，hosted raw report/人工评审/active baseline 仍待完成。G6-04P、G6-06P、G6-07P 及三个主任务都未完成。

G6-08 本地证据（2026-08-11）：release rehearsal policy/runner/workflow 合同 `10/10`；真实 macOS candidate 已完成 fresh integrity verification、5 秒 native launch 和隔离篡改 rollback，candidate 与 last-known-good 未被改写。registry clean-consumer producer、MVP schema v5 七门禁/双平台 proof、signed custody v3、readiness bundle 的外部 artifact 下载/语义验证和 protected release consumption 均有本地 fail-closed 合同；完整 release 合同保持通过。它们没有执行 registry publish、hosted signing、GitHub Release mutation 或最终发布。由于 active performance、双平台 hosted tag、registry clean-user 与 signing 前置条件尚未满足，G6-08/G6-09 保持未完成，演练不会 publish、sign、notarize 或远程触发发布。

G6 外部门禁聚合预检（2026-08-11）：新增只读 `pnpm release:preflight`，在普通脏分支也输出稳定 schema v1，而不因 release tag 不匹配提前抛栈。当前报告精确绑定 `mvp` HEAD、要求 `refs/tags/v0.1.0`，列出 final 的 8 个 pending gate、12 条成功标准缺失映射、staging/release 签名 owner/credential/execution 阻断，以及 macOS/Windows 各 6 个 pending 性能指标。合同覆盖 final、bootstrap 只排除 registry、CLI 退出码和三份 policy bytes 前后不变；预检不写 evidence、不启用 execution、不访问 registry。

G6 hosted 状态复核（2026-08-12）：只读 GitHub API 显示默认分支当前仅有旧版 CI/Docs/Native smoke/Rust/TypeScript/Dependabot workflow；本地新增的 MVP evidence、Notes package、performance、security、signing 和 publication workflow 尚未注册到远端默认分支。可见 hosted 成功 run 绑定历史 revision（最近相关 run 为 `35912970...`），没有当前 `f3bb8a0...` 的 revision-bound 记录；因此这些本地 workflow 文件和历史成功不能关闭 Windows UIA、双平台 clean artifact/picker、hosted supply-chain/performance、签名 staging 或 registry clean-user 门禁。
...
本轮本地收口门禁（2026-08-10）：Solid Tier-1 的 `solid-notes-e2e` `1/1`、release package boundary `5/5`、release manifest check 与 reviewed signing executor contracts 已通过；完整 workspace/release、Perry、Native、CLI、Clipboard 与当前九包 consumer 证据保留。性能合同同时覆盖 active baseline report 绑定、冻结采样策略、严格 CLI option parser 和采集失败与终止失败的 AggregateError 传播；这些结果不改变 G5/G6 的 hosted、registry、credential activation、signed staging、performance baseline、Clipboard/picker、Windows UIA 或 Node 22 状态。
