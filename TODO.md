...
- 状态：Desktop Notes MVP 的直接产品 hosted 门禁已闭环（PR run `31902303937` 的 41/41 jobs 覆盖 Windows UIA、双平台真实 picker/clean-runner、安全门禁、v1 active 性能预算与 G6-05 unsigned input；run `31991801398` attempt 1 已完成 `reference-notes-v3` 双平台三副本 baseline activation，run `31994708107` attempt 1 已完成 active required 复核；clean-tag schema-v5 晋级、registry clean-user、真实签名/公证和最终发布仍待外部条件）
- 规划输入：`mvp@f3afbeb`
...
- [x] G6-07B1：冻结 native event、进程驱动、RSS、schema-v2 runner report、raw-byte-bound report set 与 fail-closed 合同；验收为 `PROJECT-DESIGN.md` / `PERFORMANCE.md` 明确普通运行零影响及 local/hosted 证据边界；文件：两份设计文档；规模 S。
- [x] G6-07B2：以 RED→GREEN 合同测试冻结真实 `Presented` raw event、`GITHUB_SHA`/run ID/attempt/platform/runner/replica/artifact identity、drop/timeout/缺样本/本地证据 fail-closed，以及 artifact 前后不变、每进程 frame identity、raw SHA-256 和三副本绑定；验证：性能 Node 合同 `33/33`、Rust probe `3/3`。
...
- [x] G6-07B4：实现跨平台 Node collector；每 replica 固定 3 warmup + 10 measured、每 measured 进程 1 startup + 100 steady-state（共 1000 个 steady 样本）、monotonic first-present、5 秒后 RSS、完整 frame durations、archive/tree/executable SHA-256、artifact bytes、run/attempt/replica/commit binding、identity consistency、硬终止和 fail-closed report；frame 指标先取每进程 nearest-rank p95，再取 10 进程中位数；验证：性能 Node 合同 `33/33`，并覆盖固定 startup 边界、后续慢帧保留、active report-set 绑定、严格 CLI 参数和 collection/termination 双错误传播。
- [x] G6-07B5：将 collector 接入 `macos-15` / `windows-2022` workflow；每平台单一 producer 只构建一次候选 tar，3 个独立 hosted replica 分别 capture，platform job 以 `aggregate` / `check-set` 生成三副本中位数门禁，并在 check 回归时仍保留完整 raw/report-set artifact；`require_active` 继续阻断 pending；验证：performance workflow contract tests GREEN。
- [x] G6-07P：run `31991801398` attempt 1 已取得并逐字节复核双平台各 3 份独立 hosted raw report、完整 report set 和共享 candidate identity，激活 12 项 `reference-notes-v3` baseline；随后 `performance-required` run `31994708107` attempt 1 的 11/11 performance jobs、两个 active report-set gate 和 `CI / result` job `95290266484` 全部成功，关闭本项。
...
G6-06/07 当前状态（2026-08-17 hosted 补充）：`tools/signing-executor.mjs` v1.2.0 与 `tools/signing-credentials.mjs` 已由 `release/signing-policy.json` 绑定 entrypoint/closure SHA-256、runner 与平台命令路径；executor 在复制 unsigned bytes 或激活 credential 前强制验证规范 G6-05 descriptor、`SHA256SUMS`、CycloneDX 1.6 SBOM 与 provenance，custody schema v3 再绑定这些输入，publisher 也会独立复核双平台使用同一个完整输入 bundle。`signing.yml` 已按 `unsigned_run_id` 校验 producer run/revision、下载 canonical bundle，并编码 credential activation、签名、custody export/merge、signed manifest 与 publisher verification。两个 platform job 仍有 literal `if: ${{ false }}`，execution/credential activation 为 `disabled`，owner 仍未分配，protected Environment/真实凭据未配置，staging record 仍为 `pending`，所以 G6-06/G6-06P 按设计保持未完成。性能方面，run `31991801398` attempt 1 在 merge revision `2c91ac02340245e5bad206bc1e9fe0d8b96b88fd` 上完成 11/11 performance jobs；六份 raw、两个 report set、candidate tar/tree/executable identity 和独立 artifact bytes 已复核，12 项 v3 baseline 全部 active。required run `31994708107` attempt 1 绑定 source head `85e4383fc2aacd030b78c77bcde2fa3da460ff63` / execution revision `19d8f2894671c082a3f5b6f3883cffa4d03a38a9`，11/11 performance jobs、两个 active report-set gate 和 result job `95290266484` 全部成功，因此 G6-07/G6-07P 均已关闭。

G6-08 当前证据（2026-08-17）：release rehearsal policy/runner/workflow 覆盖 fresh integrity verification、5 秒 native launch、隔离篡改 rollback 与 promotion decision，candidate 与 last-known-good 不会被改写。registry clean-consumer producer、MVP schema v5 七门禁/双平台 proof、signed custody v3、readiness bundle 的外部 artifact 下载/语义验证和 protected release consumption 均有 fail-closed 合同。v3 active performance 已由 run `31991801398` 的三副本 report set 激活并由 run `31994708107` 完成 required gate；仍缺 clean-tag 双平台 hosted staging、registry clean-user、签名 owner/凭据与真实 signed custody，因此 G6-08/G6-09 保持未完成，candidate 演练不会 publish、sign、notarize 或远程触发发布。
...
G6 hosted 状态复核（2026-08-17）：PR run `31902303937` 的 41/41 jobs 已绑定 source head `e56bb9e2c531e9cd3d97837465eca92d5e2c31dd`；Actions 中的 package/performance/artifact 执行则绑定 merge revision `991b28142783833c14be659125c4564d14219cc5`。该 run 关闭 Windows UIA、双平台 clean artifact/picker、hosted supply-chain、v1 performance 与 unsigned G6-05 input 的直接门禁。v2 run `31958001217` 作为历史 steady-state 证据保留；v3 capture run `31991801398` attempt 1 绑定 source head `8302b89200698df6a6ead7772928107ba4d56eee` / merge revision `2c91ac02340245e5bad206bc1e9fe0d8b96b88fd`，11/11 performance jobs 生成并复核六份 raw 和两个 report set，完成 v3 activation。required run `31994708107` attempt 1 绑定 source head `85e4383fc2aacd030b78c77bcde2fa3da460ff63` / execution revision `19d8f2894671c082a3f5b6f3883cffa4d03a38a9`，11/11 performance jobs 与 result job `95290266484` 全部成功，关闭 G6-07P；这些 PR run 都不是 clean tag，且没有 credential activation、registry clean-user、signed staging 或最终 publication，因此外部门禁继续保持未完成。
...
本轮确定性收口证据保留 Solid Tier-1 `solid-notes-e2e`、release package boundary、release manifest、reviewed signing executor、workspace/release、Perry、Native、CLI、Clipboard 与九包 consumer 合同。run `31902303937` 已补齐 Clipboard/picker、Windows UIA、供应链与双平台 hosted 证据，run `31991801398` 补齐 v3 三副本 baseline activation，run `31994708107` 补齐 active required 复核；仍不改变 registry、credential activation、signed staging、clean-tag rehearsal 或最终发布状态。
