...
- [ ] G6-04：本地安全策略与 workflow 已实现，三份 lockfile 的 `cargo audit --deny warnings` 和三个 manifest 的 `cargo-deny 0.20.2` 已通过；仍缺绑定 revision 的 hosted npm registry audit、Cargo advisory/deny 与 secret-scan 成功记录。
- [x] G6-05：checksum、CycloneDX 1.6 dependency graph 和 SLSA provenance 的生成/离线 fresh verify 合同已覆盖当前 9 个 npm 根与 2 个 Cargo Host 根。
...
**G6-04 本地供应链补充（2026-08-11）**：真实 `cargo audit --deny warnings` 发现根 workspace 与 UI Host 的 `winit` 默认 feature 通过 `wayland-csd-adwaita -> sctk-adwaita -> ab_glyph -> owned_ttf_parser -> ttf-parser` 命中 `RUSTSEC-2026-0192`。workspace 现禁用 `winit` 默认 feature，并显式保留 `rwh_06`、Wayland、Wayland 动态加载、X11 和 `wayland-csd-adwaita-notitle`；这保留 Linux 窗口后端与 CSD，同时移除只服务标题文字的废弃字体解析链。根/UI Host lockfile 不再含该链，三份 lockfile 的 `cargo audit 0.22.2` 与三个 manifest 的 `cargo-deny 0.20.2` advisories/bans/licenses/sources 均通过。期间还以 RED→GREEN 合同修正 `security.yml` 的 `cargo deny` 全局 `--config` 参数位置；这些本地结果不替代 hosted npm/gitleaks/Cargo revision-bound 证据。
