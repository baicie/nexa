...
- UI 与 System 使用独立的 feature namespace 和 error code domain；未知 optional feature 忽略，未知或缺失的 required feature 结构化失败。
- 逻辑句柄是 HandleRef { slot: u32, generation: u32 }。FFI 通过 Perry jsvalue 传递一个只含这两个数值字段的 JS 对象；不通过 JS number 传 packed u64。
...
1. Host 先验证 protocol.major 和 abi.major。任一不同都返回 PROTOCOL_MISMATCH，不得启动半兼容 session。
2. 相同 major 时，双方采用 min(client.minor, host.minor) 作为共同 minor。更高 minor 只能使用对应 feature bit 标记的可选能力；基础字段必须保持兼容。
3. patch 只用于诊断和 bug-fix 追踪，不参与拒绝条件，也不得改变字段类型、字段含义、枚举数值或错误语义。
4. 对每个 namespace，enabled = hostSupported & (client.required | client.optional)。missingRequired = client.required & ~hostSupported 非空时返回 UNSUPPORTED_FEATURE。
5. 未知 optional bit 被忽略并从 enabled 中清除。未知 required bit 按不支持处理，返回结构化错误；不能用“忽略未知 bit”绕过必需能力检查。
6. ui 的 bit 不得在 system namespace 复用，反之亦然。targetTriple 只提供诊断；真正的平台能力必须通过 feature 协商。
7. Handshake 成功后，session 固定协商结果；运行中不得改变 protocol major、abi major 或 enabled bits。能力变化必须关闭 session 并重新握手。

### 3.3 Manifest registry

protocol/nui-host.json 和 protocol/system-host.json 是唯一数值源。每个 ID/feature 项至少包含：

    id: stable integer
    name: stable symbolic name
    introducedMinor: integer
    deprecatedSince: integer | null
    tombstone: boolean
...
- 产生句柄的 function 使用 Perry manifest returns: "jsvalue"，返回固定 shape 的 {slot, generation} object；
- 接收句柄的 function 使用 params: ["jsvalue", ...]，Rust 侧从 NaN-boxed JsValue 读取两个字段并做严格整数范围检查；
...
固定 Perry revision 06137858dc8c6f80975238377138f2f948d6ef88 的证据是：perry-ffi 提供 JsValue、object shape allocation 和 field setters；native manifest 支持 jsvalue/u32，并明确拒绝 pod 作为 return。G1-08 必须以最大 slot/generation fixture 做真实 Perry round-trip，不能只测 Rust codec。
...
客户端提供的 object 只用于定位 slot/generation，不可信任其中的 kind、owner 或 state。每个命令按期望 kind、当前 owner 和允许状态验证后才可修改资源。

generation 从 1 开始，释放后递增；generation 溢出时永久退休该 slot，禁止回绕到旧值。Rust 内部 NodeId 可以继续采用 (generation << 32) | slot，codec 必须先拆成两个 u32 再过边界。
...
- close/cancel 幂等：对当前 generation 的 Closed 句柄重复调用返回成功 no-op；
- 非关闭操作命中 Closed/Invalidated 返回 INVALID_STATE；
- generation 不匹配返回 STALE_HANDLE，不得静默忽略、panic 或操作新资源；
...
- Node/Property/Event/Command/Permission/Task/Resource/Error/Feature 的数值 ID 一经发布永不复用；移除项保留 tombstone: true。
...
- 项目至少在一个 minor 版本中标记 deprecated_since 并给出替代项，下一 major 才能移除；tombstone 仍保留以防编号复用。
...
| abi 0.x | abi 0.(x+1) | 仅在 manifest/ABI 规则允许且 required features 满足时接受 |
...
1. 将两个 protocol manifest、JSON schema 和 ID registry 加入仓库；
2. 生成 Rust/TypeScript/Perry declarations，并在 CI 做重复运行和 drift 校验；
3. 在 bridge 中增加 HandleRef codec、shape validation、NexaResult/NexaError 构造器；
4. 新增 handshake export 和 TS wrapper，先在 Perry smoke 中覆盖 major mismatch、feature intersection 和 required missing；
5. 把 Node/Task/Resource 的创建和操作迁移到 jsvalue HandleRef；完成最大 slot/generation round-trip 后再删除 asU64；
...
- Handle object 比单个数字多一次对象分配和字段校验；P0 优先正确性，后续可通过缓存 shape、批量 command 和 profile 数据优化；
...
| UI/System feature namespace 分离，unknown/required 规则明确 | 通过 |
| Handle 的宽度、null、generation、owner、kind、state 明确 | 通过 |
| 固定 Perry revision 的实际 ABI 能力已核验，禁止 POD return 误用 | 通过 |
