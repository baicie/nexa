...
`packages/nui-host/Cargo.lock` 与 `packages/system-host/Cargo.lock` 都必须提交。它们是独立 staticlib crate 的可重复依赖快照，不受根 `Cargo.lock` 覆盖。
...
2. 同时更新根 Perry CLI 版本、两个 `perry-ffi` revision 和两份 Cargo lock。
