...
test("validation matrix classifies every workspace check", () => {
  ...
  assert.equal(report.projectCount, 27);
  assert.equal(report.classifiedCheckCount, 108);
  assert.deepEqual(report.unclassified, []);
});

const expectedPlans = {
  typecheck: { runnable: 26, skipped: 1, inherited: 0 },
  test: { runnable: 7, skipped: 20, inherited: 0 },
  build: { runnable: 21, skipped: 6, inherited: 0 },
  lint: { runnable: 1, skipped: 0, inherited: 26 },
};
