...
test("the Windows driver budget covers Add-Type startup, discovery, and close", () => {
  ...
  assert.match(source, /@\("FileNameControlHost"\)/u);
  assert.match(source, /Find-AutomationNativeControlInContainers/u);
  assert.match(source, /\.Current\.ClassName/u);
  assert.match(source, /\.Current\.NativeWindowHandle/u);
  assert.match(source, /ValuePattern\]::Pattern/u);
  assert.match(source, /LegacyIAccessiblePattern\]::Pattern/u);
  assert.match(source, /Find-AutomationPatternCandidatesInContainers/u);
  assert.match(source, /foreach \(\$fileNameAutomation in \$fileNameAutomations\)/u);
  assert.match(source, /\.SetValue\(\$SelectionPath\)/u);
  assert.match(source, /\.Current\.Value -ceq \$SelectionPath/u);
  assert.match(source, /InvokePattern\]::Pattern/u);
  assert.match(source, /\.Invoke\(\)/u);
  assert.match(source, /\.DoDefaultAction\(\)/u);
